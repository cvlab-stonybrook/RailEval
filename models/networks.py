...
def define_classifier(classifier, num_classes=8, init_type='normal', init_gain=0.02, gpu_ids=[]):
    net = None
    if classifier == 'alexnet':
        ...
        net = models.vgg19_bn(weights=models.VGG19_BN_Weights.DEFAULT)
        net.classifier[6] = nn.Linear(4096, num_classes)
        ...
        net.fc = nn.Linear(2048, num_classes)
    elif classifier == 'resnet152':
        net = models.resnet152(weights=models.ResNet152_Weights.DEFAULT)
        net.fc = nn.Linear(2048, num_classes)
    elif classifier == 'resnext50':
        net = models.resnext50_32x4d(weights=models.ResNeXt50_32X4D_Weights.DEFAULT)
        net.fc = nn.Linear(2048, num_classes)
    elif classifier == 'resnext101':
        net = models.resnext101_32x8d(weights=models.ResNeXt101_32X8D_Weights.DEFAULT)
        net.fc = nn.Linear(2048, num_classes)
    elif classifier == 'densenet121':
        net = models.densenet121(weights=models.DenseNet121_Weights.DEFAULT)
        net.classifier = nn.Linear(1024, 8)
    elif classifier == 'densenet161':
        net = models.densenet161(weights=models.DenseNet161_Weights.DEFAULT)
        net.classifier = nn.Linear(2208, 8)
    elif classifier == 'densenet169':
        net = models.densenet169(weights=models.DenseNet169_Weights.DEFAULT)
        net.classifier = nn.Linear(1664, 8)
    elif classifier == 'densenet201':
        net = models.densenet201(weights=models.DenseNet201_Weights.DEFAULT)
        net.classifier = nn.Linear(1920, 8)
    elif classifier == 'efficients':
        net = models.efficientnet_v2_s(weights=models.EfficientNet_V2_S_Weights.DEFAULT)
        net.classifier[1] = nn.Linear(1280, 8)
    elif classifier == 'efficientm':
        net = models.efficientnet_v2_m(weights=models.EfficientNet_V2_M_Weights.DEFAULT)
        net.classifier[1] = nn.Linear(1280, 8)
    elif classifier == 'efficientl':
        ...
        net.classifier[1] = nn.Linear(1280, 8)
    elif classifier == 'mobilenetv3large':
        net = models.mobilenet_v3_large(weights=models.MobileNet_V3_Large_Weights.DEFAULT)
        net.classifier[3] = nn.Linear(1280,8)
    elif classifier == 'mobilenetsmall':
        net = models.mobilenet_v3_small(weights=models.MobileNet_V3_Small_Weights.DEFAULT)
        net.classifier[3] = nn.Linear(1024,8)
    elif classifier == 'vit':
        net = models.vit_b_16(weights=models.ViT_B_16_Weights.DEFAULT)
        net.heads[0] = nn.Linear(768, 8)
    elif classifier == 'swint':
        net = models.swin_t(weights=models.Swin_T_Weights.DEFAULT)
        net.head = nn.Linear(768,8)
    elif classifier == 'swins':
        net = models.swin_s(weights=models.Swin_S_Weights.DEFAULT)
        net.head = nn.Linear(768,8)
    elif classifier == 'swinb':
        net = models.swin_b(weights=models.Swin_B_Weights.DEFAULT)
        net.head = nn.Linear(1024,8)
    else:
        raise NotImplementedError('Classifier model name [%s] is not recognized' % classifier)
    return init_net(net, init_type, init_gain, gpu_ids)    


def define_stn(size=(1200,1600),gpu_ids=[]):
    net = STNet(size)
    if len(gpu_ids) > 0:
        assert(torch.cuda.is_available())
        net.to(gpu_ids[0])
...
def mobilenet_segmentator(bbone_cfg, decoder_cfg, head_cfg, pretrained_path=None, gpu_ids=[]):
    net = Segmentator(bbone_cfg, decoder_cfg, head_cfg, pretrained_path)
    if len(gpu_ids) > 0:
        assert(torch.cuda.is_available())
        net.to(gpu_ids[0])
        net = torch.nn.DataParallel(net, gpu_ids)  # multi-GPUs
...
class STNet(nn.Module):
    def __init__(self, input_size=(1200,1600), style='separate'):
        super(STNet, self).__init__()
        # Spatial transformer localization-network
        self.style = style
        self.output_h = round(input_size[0] / 256)
        self.output_w = round(input_size[1] / 256)
        ...
        self.fc_loc = nn.Sequential(
            nn.Linear(32 * self.output_h * self.output_w, 32),
            nn.ReLU(True),
            nn.Linear(32, 3 * 2),
        )
    ...
    def forward(self, x):
        ...
        xs = xs.view(-1, 32 * self.output_h * self.output_w)
